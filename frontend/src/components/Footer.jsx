  ...
  <div className="flex flex-col gap-4">
    ...
    <div className="flex flex-col gap-2">
      {footerData.quickLinks.map((link) => (
        <a key={link} href="#" className={`uppercase tracking-widest text-xs font-bold transition-colors ${isDark ? 'text-slate-500 hover:text-cyan-300' : 'text-slate-500 hover:text-[#006398]'}`}>{link}</a>
      ))}
    </div>
  </div>
);

const FooterSocial = ({ isDark }) => (
  <div className="flex flex-col gap-4">
...
const Footer = () => {
  ...
  return (
    <footer className={`w-full py-12 px-8 border-t transition-colors ${isDark ? 'bg-slate-950 border-[#45464d]/10' : 'bg-slate-50 border-gray-200'}`}>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
        <FooterBrand isDark={isDark} />
        <FooterLinks isDark={isDark} />
        <FooterSocial isDark={isDark} />
      </div>
      <div className={`max-w-7xl mx-auto mt-12 pt-8 border-t text-center ${isDark ? 'border-[#45464d]/5' : 'border-gray-200'}`}>
        <p className={`font-body text-sm uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{footerData.copyright}</p>
      </div>
    </footer>
  );
};
