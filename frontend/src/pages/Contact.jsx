...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      setForm({ name: '', email: '', company: '', phone: '', service_interest: '', message: '' });
    } catch {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <motion.div initial="hidden" animate="visible" variants={fadeUp}>
            ...
            <h1 className={`text-5xl md:text-6xl font-headline font-bold tracking-tighter mb-8 ${t.textOnSurface}`}>
              Let's Build<br />
              <span className={t.textGradient}>Something Great.</span>
            </h1>
            ...
            <div className="space-y-6">
              {[
                ...
                <motion.div key={i} variants={fadeUp} custom={i + 1} className="flex items-center gap-4">
            ...
            <div className={`rounded-2xl p-8 border ${t.isDark ? 'bg-[#171f33] border-[#45464d]/20' : 'bg-white border-gray-200 shadow-lg'}`}>
              {success ? (
                <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-16">
                  ...
                  <button onClick={() => setSuccess(false)} className={`mt-6 px-6 py-2 rounded-lg font-bold text-sm ${t.ctaGradient}`}>
                    Send Another
                  </button>
                </motion.div>
              ) : (
